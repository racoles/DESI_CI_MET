...
#Note: in future, row versus columns equal to x and y will be different for NESW cameras
        ...
        faah.pageLogging(consoleLog, logFile, "CS5 CCD ORIGIN\n\n" +
                         ...
                         format(fC.trianglePonitCCDLocationsCS5[self.CCDSelection + "A"][0] - (xCenGMSA + xOffsetA) * pixelSize + "\n" +
                         "    CS5 CCD Origin Y(A) = " + format(fC.trianglePonitCCDLocationsCS5[self.CCDSelection + "A"][1], '.3f') - ((format(yCenGMSA, '.3f') + str(yOffsetA)) * pixelSize), '.3f') + "\n\n" +
                         "    CS5 CCD Origin X(B) = " + format(fC.trianglePonitCCDLocationsCS5[self.CCDSelection + "B"][0], '.3f') - ((format(xCenGMSB, '.3f') + str(xOffsetB)) * pixelSize), '.3f') + "\n" +
